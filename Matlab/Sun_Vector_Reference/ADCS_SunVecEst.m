...
lon_S_ec = S_lon_m + 1.914666471*sind(M_s) + 0.019994643*sind(2*M_s); % deg
...
r_S = 1.000140612 - 0.016708617*cosd(M_s) - 0.000139589*cosd(2*M_s); % AU

% Vector in the J2000 ECI frame (assuming "mean equinox" is synonymous with
% vernal equinox) (Markley 11.52)

r_S_I = [              r_S*cosd(lon_S_ec);
         r_S*cosd(epsilon)*sind(lon_S_ec);
         r_S*sind(epsilon)*sind(lon_S_ec)]'; % AU
     

% Convert from AU to kilometres 
r_S_Ikm = r_S_I.*(149597870700)*1e-3; % 149597870700 metres = 1 AU

% Obtain unit vector:
% e_ES = r_S_I/r_S; % m

% Output
fprintf('\n  r_Earth->Sun (AU)             r_Earth->Sun (km)\n\n |   %9.8f   |       |   %9.7f    |\n |   %9.8f   |       |   %9.7f   |\n |   %9.8f   |       |   %9.7f    |\n\n', r_S_I(1),r_S_Ikm(1),r_S_I(2),r_S_Ikm(2), r_S_I(3),r_S_Ikm(3))
